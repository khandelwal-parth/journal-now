...
export default function JournalPage() {
  ...
  const saveEntry = useCallback(async (auto = false) => {
    ...
    await fetch('/api/entries', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ entry_key: currentKey, html }) });
  }, [currentKey]);
  ...
  async function deleteEntryByKey(key) {
    ...
    await fetch('/api/entries', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ entry_key: key }) });
  }
  ...
  return (
    <>
      <link href="https://fonts.googleapis.com/css2?family=Lora:ital,wght@0,400;0,600;1,400&family=DM+Mono:wght@400;500&family=Instrument+Sans:wght@400;500;600&display=swap" rel="stylesheet" />
      <style>{`
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', background:'#faf7f2' }}>
          {view === 'welcome' ? (
            ...
            <>
              ...
              <div style={{ padding:'7px 52px', borderTop:'1px solid #e8e0d4', display:'flex', alignItems:'center', justifyContent:'space-between', background:'#f2ede4', fontFamily:"'DM Mono',monospace", fontSize:11, color:'#a08c78', flexShrink:0 }}>
