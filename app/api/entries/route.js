...
export async function GET() {
  ...
  const entries = await sql`
    SELECT entry_key, html, title, created_at, updated_at
    FROM entries
...
export async function POST(req) {
  ...
  const { entry_key, html, title } = await req.json();
  if (!entry_key) return NextResponse.json({ error: 'entry_key required' }, { status: 400 });

  await sql`
    INSERT INTO entries (user_id, entry_key, html, title, updated_at)
    VALUES (${user.id}, ${entry_key}, ${html}, ${title || null}, NOW())
    ON CONFLICT (user_id, entry_key)
    DO UPDATE SET html = ${html}, title = ${title || null}, updated_at = NOW()
  `;
